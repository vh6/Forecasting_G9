...
autoplot(fc, Canton.Ticino_ts) +
  ggtitle("Forecast of Monthly Visitors to Ticino from the Philippines") +
  ylab("Forecasted Value") +
  xlab("Date")
...
After modeling the forecasts for both Vaud and Ticino, we will opt for our first model, the automatic ARIMA, for Vaud. Firstly, this model is simpler than our exogenous variables model, and we generally prefer to use simpler models unless our complex model is proven to have superior forecasts. Although we don't have future data to validate our forecasts, we can still evaluate the models using indicators such as AIC, BIC, and other error metrics.

For Vaud, the ARIMA without exogenous variables model provided us with the lowest AIC (1870.05) and BIC (1883.25), indicating a better fit compared to the ARIMA with exogenous variables and the ETS model. Additionally, this model had the lowest error metrics, including a Mean Absolute Percentage Error (MAPE) of 4.22%. These results suggest that the simpler ARIMA model is more suitable for forecasting in Vaud.
